...
import fr.artemis.phone.components.advancedrecyclerview.anim.animation.ItemAnimationInfo;
...
public abstract class BaseItemAnimationManager<T extends ItemAnimationInfo> {
	...
	private final List<T> mPending;

	private final List<List<T>> mDeferredReadySets;

	private final List<RecyclerView.ViewHolder> mActive;

	BaseItemAnimationManager( @NonNull BaseItemAnimator itemAnimator ) {
	...
	public void runPendingAnimations( boolean deferred, long deferredDelay ) {
		...
		if ( deferred ) {
			...
			RecyclerView.ViewHolder holder = ready.get( 0 ).getAvailableViewHolder();
			if ( null != holder ) {
	...
	public void endPendingAnimations( @Nullable RecyclerView.ViewHolder item ) {
		...
		for ( int i = pending.size() - 1; i >= 0; i-- ) {
			...
			if ( null != item && endNotStartedAnimation( info, item ) ) {
				pending.remove( i );
			}
		}
	...
	public void createAnimation( @NonNull T info ) {
	...
	public void resetAnimation( @NonNull RecyclerView.ViewHolder holder ) {
	...
	public void enqueuePendingAnimationInfo( @NonNull T info ) {
		mPending.add( info );
	}

	public void startActiveItemAnimation( @NonNull T info, @NonNull RecyclerView.ViewHolder holder,
	...
	public void addActiveAnimationTarget( RecyclerView.ViewHolder item ) {
		if ( item == null ) {
	...
	protected static class BaseAnimatorListener implements ViewPropertyAnimatorListener {
		...
		private BaseAnimatorListener( BaseItemAnimationManager manager, ItemAnimationInfo info,
