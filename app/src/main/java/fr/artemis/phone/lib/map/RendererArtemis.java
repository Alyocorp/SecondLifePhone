...
import fr.artemis.phone.lib.map.geojson.GeoJsonFeatureArtemis;
import fr.artemis.phone.lib.map.geojson.GeoJsonGeometryCollectionArtemis;
import fr.artemis.phone.lib.map.geojson.GeoJsonLineStringArtemis;
import fr.artemis.phone.lib.map.geojson.GeoJsonLineStringStyleArtemis;
import fr.artemis.phone.lib.map.geojson.GeoJsonMultiLineStringArtemis;
import fr.artemis.phone.lib.map.geojson.GeoJsonMultiPointArtemis;
import fr.artemis.phone.lib.map.geojson.GeoJsonMultiPolygonArtemis;
import fr.artemis.phone.lib.map.geojson.GeoJsonPointArtemis;
import fr.artemis.phone.lib.map.geojson.GeoJsonPointStyleArtemis;
import fr.artemis.phone.lib.map.geojson.GeoJsonPolygonArtemis;
...
import fr.artemis.phone.lib.map.kml.KmlMultiGeometryArtemis;
import fr.artemis.phone.lib.map.kml.KmlPlacemarkArtemis;
import fr.artemis.phone.lib.map.kml.KmlPointArtemis;
import fr.artemis.phone.lib.map.kml.KmlStyleArtemis;

public class RendererArtemis {
	...
	private final BiMultiMap<FeatureArtemis> mFeatures = new BiMultiMap<>();

	private HashMap<String, KmlStyleArtemis> mStyles;

	private HashMap<String, KmlStyleArtemis> mStylesRenderer;

	private HashMap<String, String> mStyleMaps;

	private BiMultiMap<FeatureArtemis> mContainerFeatures;

	private HashMap<KmlGroundOverlayArtemis, GroundOverlay> mGroundOverlays;
	...
	private ArrayList<KmlContainerArtemis> mContainers;

	private final GeoJsonPointStyleArtemis mDefaultPointStyle;

	private final GeoJsonLineStringStyleArtemis mDefaultLineStringStyle;

	private final GeoJsonPolygonStyleArtemis mDefaultPolygonStyle;
	 ...
	 */
	public RendererArtemis( GoogleMap map, Context context ) {
		mMap = map;
		mContext = context;
		mLayerOnMap = false;
	 ...
	 */
	public RendererArtemis( GoogleMap map, HashMap<? extends FeatureArtemis, Object> features ) {
		mMap = map;
		mFeatures.putAll( features );
		mLayerOnMap = false;
		mMarkerIconUrls = null;
		mDefaultPointStyle = new GeoJsonPointStyleArtemis();
		mDefaultLineStringStyle = new GeoJsonLineStringStyleArtemis();
		mDefaultPolygonStyle = new GeoJsonPolygonStyleArtemis();
		mImagesCache = null;
	...
	protected void putContainerFeature( Object mapObject, FeatureArtemis placemark ) {
	 ...
	 */
	public Set<FeatureArtemis> getFeatures() {
	 ...
	 */
	public FeatureArtemis getFeature( Object mapObject ) {
	 ...
	 */
	protected HashMap<? extends FeatureArtemis, Object> getAllFeatures() {
		return mFeatures;
	}
	 ...
	 */
	public HashMap<String, KmlStyleArtemis> getStylesRenderer() { return mStylesRenderer; }
	 ...
	 */
	public HashMap<KmlGroundOverlayArtemis, GroundOverlay> getGroundOverlayMap() { return mGroundOverlays; }
	 ...
	 */
	public ArrayList<KmlContainerArtemis> getContainerList() { return mContainers; }
	 ...
	 */
	protected KmlStyleArtemis getPlacemarkStyle( String styleId ) {
		KmlStyleArtemis style = mStylesRenderer.get( null );
	 ...
	 */
	public void putFeatures( FeatureArtemis feature, Object object ) {
	 ...
	 */
	public void putStyles( HashMap<String, KmlStyleArtemis> styles ) {
	 ...
	 */
	protected static void removeFeatures( HashMap<FeatureArtemis, Object> features ) {
	 ...
	 */
	public void removeFeature( FeatureArtemis feature ) {
	 ...
	 */
	private void setFeatureDefaultStyles( GeoJsonFeatureArtemis feature ) {
	 ...
	 */
	protected void storeData( HashMap<String, KmlStyleArtemis> styles,
							  HashMap<String, String> styleMaps,
							  HashMap<KmlPlacemarkArtemis, Object> features, ArrayList<KmlContainerArtemis> folders,
							  HashMap<KmlGroundOverlayArtemis, GroundOverlay> groundOverlays ) {
		mStyles = styles;
	 ...
	 */
	public void addFeature( FeatureArtemis feature ) {
		Object mapObject = FEATURE_NOT_ON_MAP;
		if ( feature instanceof GeoJsonFeatureArtemis ) {
			setFeatureDefaultStyles( (GeoJsonFeatureArtemis) feature );
		}
		if ( mLayerOnMap ) {
			if ( mFeatures.containsKey( feature ) ) {
			...
			if ( feature.hasGeometry() ) {
				...
				if ( feature instanceof KmlPlacemarkArtemis ) {
					boolean isPlacemarkVisible = getPlacemarkVisibility( feature );
					String placemarkId = feature.getId();
					Geometry geometry = feature.getGeometry();
					KmlStyleArtemis style = getPlacemarkStyle( placemarkId );
					KmlStyleArtemis inlineStyle = ( (KmlPlacemarkArtemis) feature ).getInlineStyle();
					mapObject = addKmlPlacemarkToMap( (KmlPlacemarkArtemis) feature, geometry, style, inlineStyle, isPlacemarkVisible );
				} else {
	...
	public void addProperty( FeatureArtemis feature, String property, Object value ) {
	 ...
	 */
	protected Object addGeoJsonFeatureToMap( FeatureArtemis feature, Geometry geometry ) {
		String geometryType = geometry.getGeometryType();
		switch ( geometryType ) {
			case "Point":
				MarkerOptions markerOptions = null;
				if ( feature instanceof GeoJsonFeatureArtemis ) {
					markerOptions = ( (GeoJsonFeatureArtemis) feature ).getMarkerOptions();
				} else if ( feature instanceof KmlPlacemarkArtemis ) {
					markerOptions = ( (KmlPlacemarkArtemis) feature ).getMarkerOptions();
				}
				return addPointToMap( markerOptions, (GeoJsonPointArtemis) geometry );
			case "LineString":
				PolylineOptions polylineOptions = null;
				if ( feature instanceof GeoJsonFeatureArtemis ) {
					polylineOptions = ( (GeoJsonFeatureArtemis) feature ).getPolylineOptions();
				} else if ( feature instanceof KmlPlacemarkArtemis ) {
					polylineOptions = ( (KmlPlacemarkArtemis) feature ).getPolylineOptions();
				}
				return addLineStringToMap( polylineOptions, (GeoJsonLineStringArtemis) geometry );
			case "Polygon":
				PolygonOptions polygonOptions = null;
				if ( feature instanceof GeoJsonFeatureArtemis ) {
					polygonOptions = ( (GeoJsonFeatureArtemis) feature ).getPolygonOptions();
				} else if ( feature instanceof KmlPlacemarkArtemis ) {
					polygonOptions = ( (KmlPlacemarkArtemis) feature ).getPolygonOptions();
				}
				return addPolygonToMap( polygonOptions, (DataPolygon) geometry );
			case "MultiPoint":
				return addMultiPointToMap( ( (GeoJsonFeatureArtemis) feature ).getPointStyle(), (GeoJsonMultiPointArtemis) geometry );
			case "MultiLineString":
				return addMultiLineStringToMap( ( (GeoJsonFeatureArtemis) feature ).getLineStringStyle(), ( (GeoJsonMultiLineStringArtemis) geometry ) );
			case "MultiPolygon":
				return addMultiPolygonToMap( ( (GeoJsonFeatureArtemis) feature ).getPolygonStyle(), ( (GeoJsonMultiPolygonArtemis) geometry ) );
			case "GeometryCollection":
				return addGeometryCollectionToMap( ( (GeoJsonFeatureArtemis) feature ), ( (GeoJsonGeometryCollectionArtemis) geometry ).getGeometries() );
		}
		return null;
	}
	 ...
	 * of either objects
	 */
	protected Object addKmlPlacemarkToMap( KmlPlacemarkArtemis placemark, Geometry geometry, KmlStyleArtemis style,
										   KmlStyleArtemis inlineStyle, boolean isVisible ) {
		String geometryType = geometry.getGeometryType();
		...
		if ( hasDrawOrder ) {
			try {
				drawOrder = Float.parseFloat( String.valueOf( placemark.getProperty( "drawOrder" ) ) );
		...
		switch ( geometryType ) {
			case "Point":
				...
				Marker marker = addPointToMap( markerOptions, (KmlPointArtemis) geometry );
			...
			case "LineString":
				...
				if ( inlineStyle != null ) {
					...
					polylineOptions.color( KmlStyleArtemis.computeRandomColor( polylineOptions.getColor() ) );
				}
			...
			case "Polygon":
				...
				if ( inlineStyle != null ) {
					...
					polygonOptions.fillColor( KmlStyleArtemis.computeRandomColor( polygonOptions.getFillColor() ) );
				}
				...
				return polygon;
			case "MultiGeometry":
				return addMultiGeometryToMap( placemark, (KmlMultiGeometryArtemis) geometry, style, inlineStyle,
						isVisible );
		}
		return null;
	}
	 ...
	 */
	private void setInlinePointStyle( MarkerOptions markerOptions, KmlStyleArtemis inlineStyle,
		...
		try {
			float hue = KmlStyleArtemis.getHueValue((int) inlineStyle.getCustomColorMarker());
			markerOptions.icon(BitmapDescriptorFactory.defaultMarker(hue));
		}catch ( Exception ex ) {
			Log.d("DEBUG", "On s'en tape, si google avait bien coder dès le départ, on aurai pas ce genre de crasse à faire... ");
		}
	 ...
	 * @param inlineStyle     inline styles to apply
	 */
	private void setInlineLineStringStyle( PolylineOptions polylineOptions, KmlStyleArtemis inlineStyle ) {
		...
		if ( inlineStyle.isLineRandomColorMode() ) {
			polylineOptions.color( KmlStyleArtemis.computeRandomColor( inlinePolylineOptions.getColor() ) );
		}
	}
	 ...
	 * @param inlineStyle    inline styles to apply
	 */
	private void setInlinePolygonStyle( PolygonOptions polygonOptions, KmlStyleArtemis inlineStyle ) {
		...
		if ( inlineStyle.isPolyRandomColorMode() ) {
			polygonOptions.fillColor( KmlStyleArtemis.computeRandomColor( inlinePolygonOptions.getFillColor() ) );
		}
	}
	 ...
	 */
	private ArrayList<Object> addGeometryCollectionToMap( GeoJsonFeatureArtemis feature,
	 ...
	 */
	protected static boolean getPlacemarkVisibility( FeatureArtemis feature ) {
		boolean isFeatureVisible = true;
		if ( feature.hasProperty( "visibility" ) ) {
			String placemarkVisibility = String.valueOf( feature.getProperty( "visibility" ) );
	...
	public void assignStyleMap( HashMap<String, String> styleMap,
								HashMap<String, KmlStyleArtemis> styles ) {
	 ...
	 */
	private ArrayList<Object> addMultiGeometryToMap( KmlPlacemarkArtemis placemark,
													 KmlMultiGeometryArtemis multiGeometry, KmlStyleArtemis urlStyle, KmlStyleArtemis inlineStyle,
	 ...
	 */
	private ArrayList<Marker> addMultiPointToMap( GeoJsonPointStyleArtemis pointStyle,
												  GeoJsonMultiPointArtemis multiPoint ) {
		ArrayList<Marker> markers = new ArrayList<>();
		for ( GeoJsonPointArtemis geoJsonPoint : multiPoint.getPoints() ) {
	 ...
	 */
	private ArrayList<Polyline> addMultiLineStringToMap( GeoJsonLineStringStyleArtemis lineStringStyle,
														 GeoJsonMultiLineStringArtemis multiLineString ) {
		ArrayList<Polyline> polylines = new ArrayList<>();
		for ( GeoJsonLineStringArtemis geoJsonLineString : multiLineString.getLineStrings() ) {
	...
	/**
	 * Adds all GeoJsonPolygon in the GeoJsonMultiPolygonArtemis to the map as multiple Polygons
	 *
	 ...
	 */
	private ArrayList<Polygon> addMultiPolygonToMap( GeoJsonPolygonStyleArtemis polygonStyle,
													 GeoJsonMultiPolygonArtemis multiPolygon ) {
		ArrayList<Polygon> polygons = new ArrayList<>();
		for ( GeoJsonPolygonArtemis geoJsonPolygon : multiPolygon.getPolygons() ) {
	 ...
	 */
	private void setMarkerInfoWindow( KmlStyleArtemis style, Marker marker,
									  final KmlPlacemarkArtemis placemark ) {
		...
		if ( hasBalloonOptions && hasBalloonText ) {
			...
			marker.setTitle( String.valueOf( placemark.getProperty( "name" ) ) );
			createInfoWindow();
		} else if ( hasName && hasDescription ) {
			marker.setTitle( String.valueOf( placemark.getProperty( "name" ) ) );
			marker.setSnippet( String.valueOf( placemark.getProperty( "description" ) ) );
			createInfoWindow();
		} else if ( hasDescription ) {
			marker.setTitle( String.valueOf( placemark.getProperty( "description" ) ) );
			createInfoWindow();
		} else if ( hasName ) {
			marker.setTitle( String.valueOf( placemark.getProperty( "name" ) ) );
			createInfoWindow();
...
//				return null;
//				View view =  LayoutInflater.from(mContext).inflate( com.google.maps.android.R.layout.amu_info_window, null);
//				TextView infoWindowText = (TextView) view.findViewById( com.google.maps.android.R.id.window);
