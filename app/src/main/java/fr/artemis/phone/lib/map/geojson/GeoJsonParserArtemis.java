...
public class GeoJsonParserArtemis {
	...
	private final ArrayList<GeoJsonFeatureArtemis> mGeoJsonFeatures;
	 ...
	 */
	/* package */ GeoJsonParserArtemis( JSONObject geoJsonFile ) {
	 ...
	 */
	private static GeoJsonFeatureArtemis parseFeature( JSONObject geoJsonFeature ) {
		String id = null;
		...
		return new GeoJsonFeatureArtemis( geometry, id, properties, boundingBox );
	}
	 ...
	 */
	private static GeoJsonFeatureArtemis parseGeometryToFeature( JSONObject geoJsonGeometry ) {
		Geometry geometry = parseGeometry( geoJsonGeometry );
		if( geometry != null ) {
			return new GeoJsonFeatureArtemis( geometry, null, new HashMap<String, Object>(), null );
		}
	 ...
	 */
	private static HashMap<String, Object> parseProperties( JSONObject properties )
	 ...
	 */
	private static GeoJsonPointArtemis createPoint( JSONArray coordinates ) throws JSONException {
		LatLngAlt latLngAlt = parseCoordinate( coordinates );
		return new GeoJsonPointArtemis( latLngAlt.latLng, latLngAlt.altitude );
	}
	 ...
	 * @throws JSONException if coordinates cannot be parsed
	 */
	private static GeoJsonMultiPointArtemis createMultiPoint( JSONArray coordinates ) throws JSONException {
		...
		return new GeoJsonMultiPointArtemis( geoJsonPoints );
	}
	 ...
	 * @throws JSONException if coordinates cannot be parsed
	 */
	private static GeoJsonLineStringArtemis createLineString( JSONArray coordinates ) throws JSONException {
		...
		return new GeoJsonLineStringArtemis( latLngs, altitudes );
	}
	 ...
	 * @throws JSONException if coordinates cannot be parsed
	 */
	private static GeoJsonMultiLineStringArtemis createMultiLineString( JSONArray coordinates )
	throws JSONException {
		ArrayList<GeoJsonLineStringArtemis> geoJsonLineStrings = new ArrayList<>();
		for( int i = 0 ; i < coordinates.length() ; i++ ) {
			geoJsonLineStrings.add( createLineString( coordinates.getJSONArray( i ) ) );
		}
		return new GeoJsonMultiLineStringArtemis( geoJsonLineStrings );
	}
	 ...
	 * @throws JSONException if coordinates cannot be parsed
	 */
	private static GeoJsonPolygonArtemis createPolygon( JSONArray coordinates ) throws JSONException {
		return new GeoJsonPolygonArtemis( parseCoordinatesArrays( coordinates ) );
	}
	 ...
	 * @throws JSONException if coordinates cannot be parsed
	 */
	private static GeoJsonMultiPolygonArtemis createMultiPolygon( JSONArray coordinates )
	throws JSONException {
		ArrayList<GeoJsonPolygonArtemis> geoJsonPolygons = new ArrayList<>();
		for( int i = 0 ; i < coordinates.length() ; i++ ) {
			geoJsonPolygons.add( createPolygon( coordinates.getJSONArray( i ) ) );
		}
		return new GeoJsonMultiPolygonArtemis( geoJsonPolygons );
	}
	 ...
	 */
	private static GeoJsonGeometryCollectionArtemis createGeometryCollection( JSONArray geometries )
	throws JSONException {
		...
		return new GeoJsonGeometryCollectionArtemis( geometryCollectionElements );
	}
	 ...
	 */
	private ArrayList<GeoJsonFeatureArtemis> parseFeatureCollection( JSONObject geoJsonFeatureCollection ) {
		JSONArray geoJsonFeatures;
		ArrayList<GeoJsonFeatureArtemis> features = new ArrayList<GeoJsonFeatureArtemis>();
		try {
		...
		for( int i = 0 ; i < geoJsonFeatures.length() ; i++ ) {
			try {
				...
				if( feature.getString( "type" ).equals( FEATURE ) ) {
					GeoJsonFeatureArtemis parsedFeature = parseFeature( feature );
	 ...
	 */
	/* package */ ArrayList<GeoJsonFeatureArtemis> getFeatures() {
