...
public class KmlContainerParserArtemis {
	...
	/* package */
	static KmlContainerArtemis createContainer( XmlPullParser parser, float customColorMarker )
	throws XmlPullParserException, IOException {
		return assignPropertiesToContainer( parser, customColorMarker );
	}
	 ...
	 */
	private static KmlContainerArtemis assignPropertiesToContainer( XmlPullParser parser, float customMarkerColor )
	throws XmlPullParserException, IOException {
		String startTag = parser.getName();
		String containerId = null;
		HashMap<String, String> containerProperties = new HashMap<String, String>();
		HashMap<String, KmlStyleArtemis> containerStyles = new HashMap<String, KmlStyleArtemis>();
		HashMap<? extends FeatureArtemis, Object> containerPlacemarks = new HashMap<>();
		ArrayList<KmlContainerArtemis> nestedContainers = new ArrayList<KmlContainerArtemis>();
		HashMap<String, String> containerStyleMaps = new HashMap<String, String>();
		HashMap<KmlGroundOverlayArtemis, GroundOverlay> containerGroundOverlays
				= new HashMap<KmlGroundOverlayArtemis, GroundOverlay>();
		...
		while ( !( eventType == END_TAG && parser.getName().equals( startTag ) ) ) {
			if ( eventType == START_TAG ) {
				if ( parser.getName().matches( UNSUPPORTED_REGEX ) ) {
					KmlParserArtemis.skip( parser );
				} else if ( parser.getName().matches( CONTAINER_REGEX ) ) {
					nestedContainers.add( assignPropertiesToContainer( parser, customMarkerColor ) );
					...
					setContainerStyle( parser, containerStyles, customMarkerColor );
				} else if ( parser.getName().equals( PLACEMARK ) ) {
					setContainerPlacemark( parser, (HashMap<KmlPlacemarkArtemis, Object>) containerPlacemarks, customMarkerColor );
					...
					containerGroundOverlays
							.put( KmlFeatureParserArtemis.createGroundOverlay( parser ), null );
				}
			}
			eventType = parser.next();
		}

		return new KmlContainerArtemis( containerProperties, containerStyles, (HashMap<KmlPlacemarkArtemis, Object>) containerPlacemarks,
	...
	private static void setContainerStyleMap( XmlPullParser parser,
		...
		containerStyleMap.putAll( KmlStyleParserArtemis.createStyleMap( parser ) );
	}
	...
	private static void setContainerStyle( XmlPullParser parser,
										   HashMap<String, KmlStyleArtemis> containerStyles, float customMarkerColor ) throws XmlPullParserException, IOException {
		if ( parser.getAttributeValue( null, "id" ) != null ) {
			KmlStyleArtemis style = KmlStyleParserArtemis.createStyle( parser, customMarkerColor );
	...
	private static void setContainerPlacemark( XmlPullParser parser,
											   HashMap<KmlPlacemarkArtemis, Object> containerPlacemarks, float customColorMarker )
	throws XmlPullParserException, IOException {
		containerPlacemarks.put( KmlFeatureParserArtemis.createPlacemark( parser, customColorMarker ), null );
	}
