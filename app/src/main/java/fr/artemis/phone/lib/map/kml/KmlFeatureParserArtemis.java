...
public class KmlFeatureParserArtemis {
	 ...
	 */
	/* package */
	static KmlPlacemarkArtemis createPlacemark( XmlPullParser parser, float customMarkerColor ) throws IOException, XmlPullParserException {
		String styleId = null;
		KmlStyleArtemis inlineStyle = null;
...
//					properties.putAll( setExtendedDataProperties( parser ) );
					...
					inlineStyle = KmlStyleParserArtemis.createStyle( parser, customMarkerColor );
				}
			}
			eventType = parser.next();
		}
		if ( null != properties.get( "type" ) ) {
			if ( properties.get( "blacklist" ).equals( "true" ) ) {
				inlineStyle = KmlStyleParserArtemis.createStyle( 0xff0000 );
			} else {
				if ( properties.get( "type" ).equals( "PARTIC" ) ) {
					inlineStyle = KmlStyleParserArtemis.createStyle( 0x977c00 );
				} else if ( properties.get( "type" ).equals( "PRO" ) ) {
					inlineStyle = KmlStyleParserArtemis.createStyle( 0x034900 );
				}
			}
		}
		KmlPlacemarkArtemis placemarkArtemis = new KmlPlacemarkArtemis( geometry, styleId, inlineStyle, properties );
		return placemarkArtemis;
	}

	static KmlPlacemarkArtemis createPlacemark( LatLng position, float customMarkerColor ) throws IOException, XmlPullParserException {
		String styleId = null;
		KmlStyleArtemis inlineStyle = KmlStyleParserArtemis.createStyle( customMarkerColor );
		HashMap<String, Object> properties = new HashMap<String, Object>();
		Geometry geometry = createPoint( position );
		...
		//			eventType = parser.next();
		//		}
		KmlPlacemarkArtemis placemark = new KmlPlacemarkArtemis( geometry, styleId, inlineStyle, properties );
		return placemark;
	}
	 ...
	 */
	/* package */
	static KmlGroundOverlayArtemis createGroundOverlay( XmlPullParser parser ) throws IOException, XmlPullParserException {
...
//					properties.putAll( setExtendedDataProperties( parser ) );
		...
		return new KmlGroundOverlayArtemis( imageUrl, latLonBox, drawOrder, visibility, properties, rotation );
	}
	...
	private static HashMap<String, String> setExtendedDataProperties( XmlPullParser parser ) throws XmlPullParserException, IOException {
		...
		while( ! ( eventType == END_TAG && parser.getName().matches( EXTENDED_DATA ) ) ) {
			if ( eventType == START_TAG ) {
				if ( parser.getName().equals( "matiere" ) ) {
					properties.put( parser.getName(), parser.nextText() );
...
//					properties.put( "tarifs", new HashMap<String, String>() );
//				}
//
//				HashMap<String, String> mapTarifs = properties.get( "tarifs" );
//
//				if ( parser.getName().equals( "matiere" )) {
//					key = parser.nextText();
//				} else if ( parser.getName().equals( "prix" )) {
...
//					mapTarifs.put( key, value );
//				}
//
//				properties.put( "tarifs", mapTarifs );
//				if ( parser.getName().equals( "tarif" )) {
//					if( parser.getName().equals( "matiere" ) ) {
//						key = parser.nextText();
//					} else if ( parser.getName().equals( "prix" ) ) {
//
...
//						if( mapTarifs.get() )
//
//							mapTarifs.put( parser.getName(), parser.nextText() );
//						properties.put( "tarifs", mapTarifs );
//					}
//				}
//			}
...
//		}
	 ...
	 * @return KmlPoint object
	 */
	private static KmlPointArtemis createPoint( LatLng position ) throws XmlPullParserException, IOException {
		return new KmlPointArtemis( position, 0.0 );
	}
	 ...
	 */
	private static KmlLineStringArtemis createLineString( XmlPullParser parser ) throws XmlPullParserException, IOException {
		...
		return new KmlLineStringArtemis( coordinates, altitudes );
	}
	 ...
	 */
	private static KmlTrackArtemis createTrack( XmlPullParser parser ) throws XmlPullParserException, IOException {
		...
		return new KmlTrackArtemis( latLngs, altitudes, timestamps, properties );
	}
	 ...
	 */
	private static KmlPolygonArtemis createPolygon( XmlPullParser parser ) throws XmlPullParserException, IOException {
		...
		return new KmlPolygonArtemis( outerBoundary, innerBoundaries );
	}
	 ...
	 */
	private static KmlMultiGeometryArtemis createMultiGeometry( XmlPullParser parser ) throws XmlPullParserException, IOException {
		...
		return new KmlMultiGeometryArtemis( geometries );
	}
	 ...
	 */
	private static KmlMultiTrackArtemis createMultiTrack( XmlPullParser parser ) throws XmlPullParserException, IOException {
		ArrayList<KmlTrackArtemis> tracks = new ArrayList<>();
		// Get next otherwise have an infinite loop
		int eventType = parser.next();
		...
		return new KmlMultiTrackArtemis( tracks );
	}
	 ...
	 *            coordinates string to convert from
