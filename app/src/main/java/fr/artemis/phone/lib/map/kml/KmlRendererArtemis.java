...
public class KmlRendererArtemis extends RendererArtemis {

	private static final String LOG_TAG = "KmlRendererArtemis";
	...
	/* package */ KmlRendererArtemis( GoogleMap map, Context context ) {
		super( map, context );
	 ...
	 */
	private void removePlacemarks( HashMap<? extends FeatureArtemis, Object> placemarks ) {
		// Remove map object from the map
		removeFeatures( (HashMap<FeatureArtemis, Object>) placemarks );
	}
	 ...
	 */
	static boolean getContainerVisibility( KmlContainerArtemis kmlContainer, boolean
	 ...
	 */
	private void removeGroundOverlays( HashMap<KmlGroundOverlayArtemis, GroundOverlay> groundOverlays ) {
	 ...
	 * are in a container.
	 */
	private void removeContainers( Iterable<KmlContainerArtemis> containers ) {
		for ( KmlContainerArtemis container : containers ) {
	...
	/*package*/ void storeKmlData( HashMap<String, KmlStyleArtemis> styles,
	 ...
	 */
	/* package */ Iterable<? extends FeatureArtemis> getKmlPlacemarks() {
		return ( getFeatures() );
	}
	 ...
	 */
	public Iterable<KmlContainerArtemis> getNestedContainers() {
		return mContainers;
	}
	 ...
	 * @param placemarks
	 */
	private void addPlacemarksToMap( HashMap<? extends FeatureArtemis, Object> placemarks ) {
		for ( FeatureArtemis kmlPlacemark : placemarks.keySet() ) {
	 ...
	 */
	private void addContainerGroupToMap( Iterable<KmlContainerArtemis> kmlContainers,
										 boolean containerVisibility ) {
		for ( KmlContainerArtemis container : kmlContainers ) {
	 ...
	 */
	private void addContainerObjectToMap( KmlContainerArtemis kmlContainer, boolean isContainerVisible ) {

		for ( FeatureArtemis placemark : kmlContainer.getPlacemarks() ) {
			...
			if ( placemark.getGeometry() != null ) {
				...
				KmlStyleArtemis inlineStyle = ( (KmlPlacemarkArtemis) placemark ).getInlineStyle();
				Object mapObject = addKmlPlacemarkToMap( (KmlPlacemarkArtemis) placemark, geometry,
						style, inlineStyle, isObjectVisible );
				kmlContainer.setPlacemark( (KmlPlacemarkArtemis) placemark, mapObject );
	 ...
	 * @param placemarks
	 */
	private void addIconToMarkers( String iconUrl, HashMap<KmlPlacemarkArtemis, Object> placemarks ) {
		for ( FeatureArtemis placemark : placemarks.keySet() ) {
			KmlStyleArtemis urlStyle = getStylesRenderer().get( placemark.getId() );
			KmlStyleArtemis inlineStyle = ( (KmlPlacemarkArtemis) placemark ).getInlineStyle();
			if ( "Point".equals( placemark.getGeometry().getGeometryType() ) ) {
				...
				if ( isInlineStyleIcon ) {
					scaleBitmap( inlineStyle, placemarks, (KmlPlacemarkArtemis) placemark );
				} else if ( isPlacemarkStyleIcon ) {
					scaleBitmap( urlStyle, placemarks, (KmlPlacemarkArtemis) placemark );
				}
			}
		}
	}
	 ...
	 */
	private void scaleBitmap( KmlStyleArtemis style, HashMap<KmlPlacemarkArtemis, Object> placemarks,
							  KmlPlacemarkArtemis placemark ) {
	...
	private void addContainerGroupIconsToMarkers( String iconUrl,
												  Iterable<KmlContainerArtemis> kmlContainers ) {
		for ( KmlContainerArtemis container : kmlContainers ) {
	 ...
	 */
	private void addGroundOverlays( HashMap<KmlGroundOverlayArtemis, GroundOverlay> groundOverlays,
									Iterable<KmlContainerArtemis> kmlContainers ) {
	 ...
	 */
	private void addGroundOverlays( HashMap<KmlGroundOverlayArtemis, GroundOverlay> groundOverlays ) {
		for ( KmlGroundOverlayArtemis groundOverlay : groundOverlays.keySet() ) {
	...
	private void addGroundOverlayToMap( String groundOverlayUrl,
										HashMap<KmlGroundOverlayArtemis, GroundOverlay> groundOverlays, boolean containerVisibility ) {
		BitmapDescriptor groundOverlayBitmap = BitmapDescriptorFactory
				.fromBitmap( getImagesCache().get( groundOverlayUrl ) );
		for ( KmlGroundOverlayArtemis kmlGroundOverlay : groundOverlays.keySet() ) {
	...
	private void addGroundOverlayInContainerGroups( String groundOverlayUrl,
													Iterable<KmlContainerArtemis> kmlContainers, boolean containerVisibility ) {
		for ( KmlContainerArtemis container : kmlContainers ) {
			boolean isContainerVisible = getContainerVisibility( container, containerVisibility );
	...
	private class MarkerIconImageDownload extends AsyncTask<String, Void, Bitmap> {
		...
		protected void onPostExecute( Bitmap bitmap ) {
			if ( bitmap == null ) {
				...
				if ( isLayerOnMap() ) {
					addIconToMarkers( mIconUrl, (HashMap<KmlPlacemarkArtemis, Object>) getAllFeatures() );
