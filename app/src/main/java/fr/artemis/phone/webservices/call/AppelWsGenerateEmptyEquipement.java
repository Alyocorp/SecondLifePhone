...
/**
 * Tache de creation d'un equipement vide
 */
public class AppelWsGenerateEmptyEquipement extends AbstractCaller<CRMPhoneEquipementDTO> {
	 ...
	 */
	public AppelWsGenerateEmptyEquipement( WsCaller source, Map<String, Object> mapResources, WsName name ) {
	...
	protected CRMPhoneEquipementDTO executionRequete() {
		...
		SoapObject requete = new SoapObject( ConstantesWS.NAMESPACE, ConstantesWS.METHOD_GENERATE_EQUIPEMENT );
		...
		try {
			...
			if ( null != reponse ) {
				...
				return new EquipementMapper().map( obj );
			}
		} catch ( Exception ex ) {
			ex.printStackTrace();
