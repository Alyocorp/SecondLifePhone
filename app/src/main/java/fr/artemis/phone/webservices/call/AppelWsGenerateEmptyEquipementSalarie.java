...
/**
 * Tache de creation d'une demande d'equipement de salarié vide
 */
public class AppelWsGenerateEmptyEquipementSalarie extends AbstractCaller<CRMPhoneEquipementSalarieDTO> {
	 ...
	 */
	public AppelWsGenerateEmptyEquipementSalarie( WsCaller source, Map<String, Object> mapResources, WsName name ) {
		super( source, mapResources, name );
	}

	@Override
	protected CRMPhoneEquipementSalarieDTO executionRequete() {

		// Injection des propriétés nécessaires à l'authentification
		SoapObject requete = new SoapObject( ConstantesWS.NAMESPACE, ConstantesWS.METHOD_GENERATE_EQUIPEMENT_SALARIE );
		...
		try {
			...
			if ( null != reponse ) {
				...
				return new EquipementSalarieMapper().map( obj );
			}

		} catch ( Exception ex ) {
			ex.printStackTrace();
			FirebaseCrashlytics.getInstance().recordException( ex );
			Log.e( "BUG", "Erreur lors de la creation d'un equipement vide : ", ex );
		}
		return null;
	}
}