...
/**
 * Tache de recuperation de la liste des depots vente
 */
public class AppelWsGetListeDepotsVente extends AbstractCaller<List<CRMPhoneDepotsVenteDTO>> {
	 ...
	 */
	public AppelWsGetListeDepotsVente( WsCaller source, Map<String, Object> mapResources, WsName name ) {
		super( source, mapResources, name );
	}

	@Override
	protected List<CRMPhoneDepotsVenteDTO> executionRequete() {

		// Injection des propriétés nécessaires à l'authentification
		SoapObject requete = new SoapObject( ConstantesWS.NAMESPACE, ConstantesWS.METHOD_GET_DEPOTS_VENTE );
		...
		try {
			...
			if ( null != reponse && reponse.getPropertyCount() > 0 ) {
				return new DepotVenteMapper().mapList( reponse );
			}

		} catch ( Exception ex ) {
			ex.printStackTrace();
			FirebaseCrashlytics.getInstance().recordException( ex );
			Log.e( "BUG", "Erreur lors de la recuperation des dépots vente : ", ex );
		}
		return null;
	}
}