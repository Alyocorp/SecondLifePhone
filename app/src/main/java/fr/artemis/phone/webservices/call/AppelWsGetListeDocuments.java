...
import fr.artemis.phone.webservices.mapper.impl.DocumentMapper;

/**
 * Tache de recuperation de la liste des documents
 */
public class AppelWsGetListeDocuments extends AbstractCaller<List<CRMPhoneGEDDocumentDTO>> {
	 ...
	 */
	public AppelWsGetListeDocuments( WsCaller source, Map<String, Object> mapResources, WsName name ) {
		super( source, mapResources, name );
	}

	@Override
	protected List<CRMPhoneGEDDocumentDTO> executionRequete() {

		// Injection des propriétés nécessaires à l'authentification
		SoapObject requete = new SoapObject( ConstantesWS.NAMESPACE, ConstantesWS.METHOD_GET_DOCUMENTS );
		...
		try {
			...
			if ( null != reponse && reponse.getPropertyCount() > 0 ) {
				return new DocumentMapper().mapList( reponse );
			}

		} catch ( Exception ex ) {
			ex.printStackTrace();
			FirebaseCrashlytics.getInstance().recordException( ex );
			Log.e( "BUG", "Erreur lors de la recuperation des documents : ", ex );
		}
		return null;
	}
}