...
/**
 * Appel au webservice de récuperation des equipements
 */
public class AppelWsGetListeEquipements extends AbstractCaller<List<CRMPhoneEquipementDTO>> {
	 ...
	 */
	public AppelWsGetListeEquipements( WsCaller source, Map<String, Object> mapResources, WsName name ) {
		super( source, mapResources, name );
	}

	@Override
	protected List<CRMPhoneEquipementDTO> executionRequete() {

		// Injection des propriétés nécessaires à l'authentification
		SoapObject requete = new SoapObject( ConstantesWS.NAMESPACE, ConstantesWS.METHOD_LISTE_EQUIPEMENTS );
		...
		try {
			...
			if ( null != reponse && reponse.getPropertyCount() > 0 ) {
				return new EquipementMapper().mapList( reponse );
			}
