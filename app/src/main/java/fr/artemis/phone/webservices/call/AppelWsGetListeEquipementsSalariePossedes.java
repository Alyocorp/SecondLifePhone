...
import fr.artemis.phone.webservices.mapper.impl.EquipementSalarieMapper;

/**
 * Tache de recuperation de la liste des equipements possédé par les salariés
 */
public class AppelWsGetListeEquipementsSalariePossedes extends AbstractCaller<List<CRMPhoneEquipementSalarieDTO>> {
	 ...
	 */
	public AppelWsGetListeEquipementsSalariePossedes( WsCaller source, Map<String, Object> mapResources, WsName name ) {
		super( source, mapResources, name );
	}

	@Override
	protected List<CRMPhoneEquipementSalarieDTO> executionRequete() {

		// Injection des propriétés nécessaires à l'authentification
		SoapObject requete = new SoapObject( ConstantesWS.NAMESPACE, ConstantesWS.METHOD_LISTE_EQUIPEMENTS_SALARIE_POSSEDES );
		...
		try {
			...
			if ( null != reponse && reponse.getPropertyCount() > 0 ) {
				return new EquipementSalarieMapper().mapList( reponse );
			}

		} catch ( Exception ex ) {
			ex.printStackTrace();
			FirebaseCrashlytics.getInstance().recordException( ex );
			Log.e( "BUG", "Erreur lors de la recuperation de la liste des equipements : ", ex );
		}
		return null;
	}
}