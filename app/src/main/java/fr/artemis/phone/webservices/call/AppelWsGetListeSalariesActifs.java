...
/**
 * Recuperation de la liste des salariés actifs
 */
public class AppelWsGetListeSalariesActifs extends AbstractCaller<List<SalarieLightDTO>> {
	 ...
	 */
	public AppelWsGetListeSalariesActifs( WsCaller source, Map<String, Object> mapResources, WsName name ) {
		super( source, mapResources, name );
	}

	@Override
	protected List<SalarieLightDTO> executionRequete() {

		// Injection des propriétés nécessaires à l'authentification
		SoapObject requete = new SoapObject( ConstantesWS.NAMESPACE, ConstantesWS.METHOD_LISTE_PHONE_SALARIES );
		...
		try {
			...
			if ( null != reponse && reponse.getPropertyCount() > 0 ) {
				return new SalarieMapper().mapList( reponse );
			}
