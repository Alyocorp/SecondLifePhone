...
/**
 * Tache de recuperation du numero de version de la carte des depots vente
 */
public class AppelWsGetVersionMapDepotsVente extends AbstractCaller<Integer> {
	 ...
	 */
	public AppelWsGetVersionMapDepotsVente( WsCaller source, Map<String, Object> mapResources, WsName name ) {
		super( source, mapResources, name );
	}

	@Override
	protected Integer executionRequete() {

		SoapObject requete = new SoapObject( ConstantesWS.NAMESPACE, ConstantesWS.METHOD_GET_VERSION_MAP_DEPOTS_VENTE );
		...
		try {
			...
			if ( null != reponse ) {
				return Integer.parseInt( reponse.toString() );
			}

		} catch ( Exception ex ) {
			ex.printStackTrace();
			FirebaseCrashlytics.getInstance().recordException( ex );
			Log.e( "BUG", "Erreur lors de la recuperation du numero de version de carte des depots vente : ", ex );
		}
		return null;
	}
}