...
public class AppelWsSendDocument extends AbstractCaller<Boolean> {
	 ...
	 * @param mapResources
	 *            Map contenant des resources nécessaires aux futurs traitements de
	 *            l'appelant
	 * @param name
	 *            Le nom de la tache asynchrone (permet d'identifier qui répond
	 *            lorsques plusieurs taches sont lancées en meme temps)
	 */
	public AppelWsSendDocument( WsCaller source, Map<String, Object> mapResources, WsName name ) {
		super( source, mapResources, name );
	}

	/**
	 * Injection du destinataire
	 * 
	...
	/**
	 * Injection de l'identifiant du document
	 * 
	...
	protected Boolean executionRequete() {
		...
		SoapObject requete = new SoapObject( ConstantesWS.NAMESPACE, ConstantesWS.METHOD_SEND_DOCUMENT );
		requete.addProperty( ConstantesWS.PARAM_WS_ID_DOCUMENT, idDocument );
		requete.addProperty( ConstantesWS.PARAM_WS_DESTINATAIRE, destinataire );
		...
		try {
			...
			Log.e( "BUG", "Erreur lors de l'envoi du document.", ex );
		}
		return null;
	}
}