...
import fr.artemis.phone.webservices.mapper.impl.DecheterieMapper;

/**
 * Tache de mise à jour d'une decheterie
 */
public class AppelWsUpdateDecheterie extends AbstractCaller<CRMPhoneDecheterieDTO> {

	// La decheterie
	private CRMPhoneDecheterieDTO decheterie;
	 ...
	 */
	public AppelWsUpdateDecheterie( WsCaller source, Map<String, Object> mapResources, WsName name ) {
		super( source, mapResources, name );
	}

	/**
	 * Injection de la décheterie
	 *
	 * @param decheterie
	 *            La decheterie à mettre à jour
	 */
	public void setDecheterie( CRMPhoneDecheterieDTO decheterie ) {
		this.decheterie = decheterie;
	}

	@Override
	protected CRMPhoneDecheterieDTO executionRequete() {

		SoapObject requete = new SoapObject( ConstantesWS.NAMESPACE, ConstantesWS.METHOD_SAVE_DECHETERIE );
		requete.addProperty( ConstantesWS.PARAM_WS_DECHETERIE, decheterie );
		...
		try {
			...
			if ( null != reponse ) {
				return new DecheterieMapper().map( reponse );
			}

		} catch ( Exception ex ) {
			ex.printStackTrace();
			FirebaseCrashlytics.getInstance().recordException( ex );
			Log.e( "BUG", "Erreur lors de l'enregistrement d'une decheterie : ", ex );
		}
		return null;
	}
}